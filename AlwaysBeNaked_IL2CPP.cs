...
namespace AlwaysBeNaked_IL2CPP
{
    [BepInPlugin("com.wolfitdm.AlwaysBeNaked_IL2CPP", "AlwaysBeNaked Plugin_IL2CPP", "1.0.0.0")]
    public class AlwaysBeNaked_IL2CPP : BasePlugin
    {
        internal static new ManualLogSource Log;
		internal static new ManualLogSource Logger;
		
		public AlwaysBeNaked_IL2CPP()
        {
        }
        ...
        public static Type oldMyGetType(string originalClassName)
        {
            ...
            try
            {
                switch (originalClassName)
                {
                    ...
                    case "LocationManager_UW":
                        {
                            originalClass = typeof(LocationManager_UW);
                        }
                        break;
        ...
        public static void PatchHarmonyMethod(string originalClassName, string originalMethodName, string patchedMethodName, bool usePrefix, bool usePostfix)
        {
            ...
            var harmony = new Harmony("com.wolfitdm.AlwaysBeNaked_IL2CPP");
            ...
            MethodInfo patched = AccessTools.Method(typeof(AlwaysBeNaked_IL2CPP), patchedMethodName);
        ...
        public override void Load()
        {
            Logger = base.Log;
			Log = base.Log;
            //Harmony.CreateAndPatchAll(typeof(AlwaysBeNaked_IL2CPP));
            ...
            PatchHarmonyMethodPostfix("LocationManager_UW", "LoadLevel");
            PatchHarmonyMethodPostfix("GlobalObjects_UW", "CheckVersionEXCHBA");
            Logger.LogInfo($"Plugin AlwaysBeNaked_IL2CPP is loaded!");
        }
        ...
        //[HarmonyPostfix]
        static void LoadLevel(ELocationsSub newLevel, int spawnNum, bool checkEvents, object __instance)
        {
        ...
        /*
         * 
             // Example target class in IL2CPP Unity game
    public class Player
    {
        public void TakeDamage(float amount)
        {
            Console.WriteLine($"Player took {amount} damage.");
                  ...
                  // Delegate matching the target method signature
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        private delegate void UpdateDelegate(IntPtr instance);

        // Store original method pointer
        private static UpdateDelegate _originalUpdate;

        // Keep detour alive
        private INativeDetour _updateDetour;

        // Delegate matching the native function signature
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        private delegate int AddNumbersDelegate(int a, int b);

        // Pointer to the original function (trampoline)
        private static AddNumbersDelegate _originalAddNumbers;

        // Keep a reference to the detour so it doesn't get GC'd
        private INativeDetour _detour;

                public override void Load()
                {
                    Log.LogInfo("HookExamplePlugin loaded.");

                    // Get the method pointer for UnityEngine.MonoBehaviour::Update
                    IntPtr? updateMethod = Il2CppType.From(typeof(MonoBehaviour))
                        .GetMethod("Update", 0)?.MethodHandle.GetFunctionPointer();

                    // Get function pointer (works in IL2CPP builds if method is not stripped)
                    IntPtr methodPtr = updateMethod == null ?  IntPtr.Zero : (IntPtr)updateMethod;
                    if (updateMethod == IntPtr.Zero)
                    {
                        Log.LogError("Failed to find Update method pointer.");
                        return;
                    }

                    // Create detour
                    _updateDetour = INativeDetour.CreateAndApply(
                        methodPtr,
                        new UpdateDelegate(UpdateHook),
                        out _originalUpdate
                    );

                    try
                    {
                        // Example: Suppose we know the function's address (from reverse engineering)
                        // Replace 0x12345678 with the actual address of the native function
                        IntPtr targetAddress = new IntPtr(0x12345678);

                        // Create the detour
                        _detour = INativeDetour.CreateAndApply(
                            targetAddress,
                            new AddNumbersDelegate(HookedAddNumbers),
                            out _originalAddNumbers
                        );

                        Log.LogInfo("Native hook applied successfully.");
                    }
                    catch (Exception ex)
                    {
                        Log.LogError($"Failed to hook native function: {ex}");
                    }

                    // Apply Harmony patches
                    var harmony = new Harmony("com.example.il2cpphook");
                    harmony.PatchAll();

                    Log.LogInfo("Update method hooked successfully.");
                }

                // Our hook method
                private void UpdateHook(IntPtr instance)
                {
                    try
                    {
                        // Example: Log every frame for the first 5 seconds
                       Log.LogInfo($"Update called on instance: {instance}");

                        // Call original method
                        _originalUpdate(instance);
                    }
                    catch (Exception ex)
                    {
                        Log.LogError($"Error in UpdateHook: {ex}");
                    }
                }

                // Our hooked function
                private static int HookedAddNumbers(int a, int b)
                {
                    // Log or modify parameters
                    Console.WriteLine($"[Hook] AddNumbers called with a={a}, b={b}");

                    // Optionally modify arguments
                    a += 10;

                    // Call the original function via trampoline
                    int result = _originalAddNumbers(a, b);

                    // Optionally modify the return value
                    result *= 2;

                    Console.WriteLine($"[Hook] Modified result: {result}");
                    return result;
                }

                // Example target class in the IL2CPP game
                // This is just for demonstration — in a real game, you'd find the actual class/method via dnSpy or Il2CppInspector
                [HarmonyPatch]
                public static class PlayerPatch
                {
                    // Tell Harmony which method to patch
                    static System.Reflection.MethodBase TargetMethod()
                    {

                        // Example: UnityEngine.Player::TakeDamage(float)
                        // Replace with the actual IL2CPP method you want to hook
                        // For IL2CPP, we must use reflection to get the MethodInfo
                        return AccessTools.Method(typeof(Player), "TakeDamage", new Type[] { typeof(float) });
                    }

                    // Prefix runs before the original method
                    static void Prefix(ref float damage)
                    {
                        Log.LogInfo($"Original damage: {damage}");
                        damage = 0f; // Make player invincible
                       Log.LogInfo("Damage set to zero!");
                    }
                }*/
    }
}